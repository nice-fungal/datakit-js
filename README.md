...
| `allowedDDTracingOrigins` $\color{#FF0000}{新增}$ | Array   | 否       | `[]`    | 允许注入`ddtrace` 采集器所需header头部的所有请求列表。可以是请求的origin，也可以是是正则，origin: `协议（包括：//），域名（或IP地址）[和端口号]` 例如：`["https://api.example.com", /https:\/\/.*\.my-api-domain\.com/]` |
| `trackInteractions` $\color{#FF0000}{新增}$       | Boolean | 否       | `false` | 是否开启用户行为采集                                                                                                                                                                                                     |
## 问题
