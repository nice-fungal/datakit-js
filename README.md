...
| `isServerError`                | Function | 否       | `function(request) {return false}` | 自定义的方式判断请求是否为`error`请求，callback参数为请求对应的相关返回参数： `{ isAborted: false, method:"get",response: "{...}",status: 200,url: "xxxx" }`, 如果方法返回为true则该请求相关数据会被采集为error指标      |
